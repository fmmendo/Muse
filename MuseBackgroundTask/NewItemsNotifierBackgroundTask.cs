...
{
    public sealed class NewItemsNotifierBackgroundTask : IBackgroundTask
    {
        ...
        public async void Run(IBackgroundTaskInstance taskInstance)
        {
            ...
            try
            {
                // If we can't send toast no point in doing anything...
                if (!CanSendToasts())
                {
                    _deferral.Complete();
                    return;
                ...
                // If no data has been cached no point in trying to match it
                var dataInCache = await Cache.GetItemsAsync<RssSchema>("NewsDataSource");
                if (dataInCache == null)
                {
                    _deferral.Complete();
                    return;
                ...
                using (HttpContent content = response.Content)
                {
                    ...
                    foreach (var item in xmlItems.Descendants("item"))
                    {
                        ...
                        if (dataInCache.Items.Any(i => i.Title.Contains(title)))
                            continue;
                        else
