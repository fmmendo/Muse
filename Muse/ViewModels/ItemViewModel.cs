...
namespace Muse
{
    public class ItemViewModel : INotifyPropertyChanged
    {
            ...
            set
            {
                if (value != _description)
                {
                    ...
                    if (String.IsNullOrEmpty(MonthYear))
                    {
                        ...
                        start = _description.IndexOf("<b>On:</b>");
                        end = _description.IndexOf("<br />");

                        if (start >= 0 && end > 0 && end > start)
                        {
                            string date = _description.Substring(start + 10, end - (start + 10)).Trim();
                            ...
                            //MonthYear = String.Format("{0} {1}", split[1], split[2]);
                        }
                    }
                    ...
                    if (String.IsNullOrEmpty(ImageURL))
                    {
                        ...
                        startImage = _description.IndexOf("<img src=\"", 0);
                        if (startImage >= 0)
                        {
                            startImage += 10;
                            endImage = _description.IndexOf("\" ", startImage);
                            if (endImage >= startImage)
                            {
                                endtag = _description.IndexOf(">", endImage);
                                if (endtag >= endImage)
                                {
                                    ImageThumb = _description.Substring(startImage, endImage - startImage);
                                    ImageURL = ImageThumb.Replace("thumb", "original");
                                    _description = _description.Substring(endtag + 1);
                                }
                            }
                        }
                    }
        ...
        /// <summary>
        /// Notifies View of a change to a property
        /// </summary>
